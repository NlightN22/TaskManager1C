...
class TasksFromRemoteDbImpl(
    ...
    override val listTasks: Flow<Request<List<TaskDb>>>
        get() = flow {
...
//            emit(Request.Success(tasks))
    ...
    override val listUsers: Flow<Request<List<UserDb>>>
        get() = flow {
            emit(Request.Loading())
            autoUpdate()
            emit(Request.Loading())
    ...
    override fun getUser(userId: String): Flow<Request<UserDb>> = flow {
        emit(Request.Loading())
        autoUpdate()
        emit(Request.Loading())
...
//    }
    ...
    private suspend fun saveTaskListDto(tasksRemote: TaskListDto, tasksLocal: List<TaskDb>) {
